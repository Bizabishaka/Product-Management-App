<template>
  <aside
    :class="[
      'bg-white shadow-lg flex flex-col justify-between h-screen w-64 z-20 transition-transform',
      // Positioning
      'fixed top-0 left-0 right-5',
      // Mobile slide
  ...
  >
    <div>
      ...
      <nav class="mt-4">
        <RouterLink
          to="/dashboard"
          class="flex items-center gap-3 p-4 hover:bg-gray-100 rounded"
        >
    ...
    <div class="mb-4">
      ...
      <button
        @click="logout"
        class="flex items-center gap-2 p-4 text-left w-full hover:bg-red-100 text-red-600 rounded"
      >
  ...
  <div
    v-if="isOpen"
    @click="$emit('close')"
    class="fixed inset-0 bg-black opacity-50 sm:hidden z-10"
  ></div>
</template>
